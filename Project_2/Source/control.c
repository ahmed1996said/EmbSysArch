...
volatile int g_duty_cycle=5;  // global to give debugger access changed from int16_t
...
volatile int error;
volatile int arr_gSetI[960];
volatile int arr_full=0;


extern osMessageQueueId_t requests_queue_ID;
extern osMessageQueueId_t results_queue_ID; //maybe not global??
...
void ADC0_IRQHandler() {
	...
	static enum {PRIORITY,QUEUED_REQUEST} previous_conversion;
	//static enum {S1,S2,S3,S4,S5} next_state=S1;
	static uint32_t queue_count;
	static osStatus_t result_status;
	static MSG_REQ 	 msg_to_convert;
	static MSG_RESULT msg_converted;
	...
	FPTB->PSOR = MASK(DBG_IRQ_ADC);
					...
					// check if there are requests in queue
						...
						else{ 	// start conversion for queued requests
