...
osStatus_t result_st;

extern osMessageQueueId_t requests_queue_ID;
extern osMessageQueueId_t results_queue_ID;
...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	// Read inputs
		...
		//	ADC0->SC1[0] = LCD_TS_YU_CHANNEL; // start conversion on channel YU
			msg_req.ChannelNum=LCD_TS_YU_CHANNEL;
			msg_req.ResultQueue=results_queue_ID;
			
		  result_st = osMessageQueuePut(requests_queue_ID,&msg_req,NULL,0);			
	
	//	while (!(ADC0->SC1[0] & ADC_SC1_COCO_MASK))
	//		;
			if(result_st==osOK){
		    osMessageQueueGet(results_queue_ID,&msg_res,NULL,osWaitForever);
				yu = msg_res.Result;
			}
	...
	// Read inputs
		...
		//  ADC0->SC1[0] = LCD_TS_XL_CHANNEL; // start conversion on channel XL
			...
			msg_req.ChannelNum=LCD_TS_XL_CHANNEL;
			msg_req.ResultQueue=results_queue_ID;
