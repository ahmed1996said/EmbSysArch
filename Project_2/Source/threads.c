...
osMessageQueueId_t requests_queue_ID;
osMessageQueueId_t results_queue_ID;
...
void Create_OS_Objects(void) {
	...
	requests_queue_ID = osMessageQueueNew(2, sizeof(MSG_REQ), NULL);
  results_queue_ID  = osMessageQueueNew(2, sizeof(MSG_RESULT), NULL);
