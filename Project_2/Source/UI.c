...
extern volatile int arr_gSetI[960];
extern volatile int arr_full;
...
void UI_Draw_Empty_Graph(void){
	PT_T graph={0,20};
	PT_T graph2={240,130};
  
	PT_T v_line1[2]={ {24,130},{24,20} };
	PT_T v_line2[2]={ {72,130},{72,20} };
	PT_T v_line3[2]={ {120,130},{120,20} };
	PT_T v_line4[2]={ {168,130},{168,20} };
	PT_T v_line5[2]={ {216,130},{216,20} };
	
	PT_T h_line1[2]={ {0,130},{240,130} };
	PT_T h_line2[2]={ {0,42},{240,42} };
	PT_T h_line3[2]={ {0,64},{240,64} };
	PT_T h_line4[2]={ {0,86},{240,86} };
	PT_T h_line5[2]={ {0,108},{240,108} };
	PT_T h_line6[2]={ {0,130},{240,130} };
	
	//Vertical Lines:
	LCD_Fill_Rectangle(&graph,&graph2,&black);
	
	//LCD_Draw_Line(&v_line1[0],&v_line1[1],&white);
	//LCD_Draw_Line(&v_line2[0],&v_line2[1],&white);
	LCD_Draw_Line(&v_line3[0],&v_line3[1],&white);
	//LCD_Draw_Line(&v_line4[0],&v_line4[1],&white);
	//LCD_Draw_Line(&v_line5[0],&v_line5[1],&white);
	
	//Horizontal Lines:
	LCD_Draw_Line(&h_line1[0],&h_line1[1],&white);
	//LCD_Draw_Line(&h_line2[0],&h_line2[1],&white);
	LCD_Draw_Line(&h_line3[0],&h_line3[1],&white);
	//LCD_Draw_Line(&h_line4[0],&h_line4[1],&white);
	//LCD_Draw_Line(&h_line5[0],&h_line5[1],&white);
	//LCD_Draw_Line(&h_line6[0],&h_line6[1],&white);
...
void UI_Plot(void){
	...
	//LCD_Fill_Rectangle(&graph,&graph2,&black);		
	for(int i=0;i<960;i++)
		LCD_Plot_Pixel(&(PT_T){(i)/4,130-(arr_gSetI[i])},&blue);
		arr_full=0;
	}
